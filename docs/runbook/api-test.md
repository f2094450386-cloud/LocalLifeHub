...
```powershell
$body = @{
  message = "帮我查一下 103茶餐厅 的优惠券"
  sessionId = "demo-session-stream"
} | ConvertTo-Json

Invoke-WebRequest `
  -Method Post `
  -Uri "http://127.0.0.1:8081/ai/customer-service/chat/stream" `
  -Headers @{ authorization = $token } `
  -ContentType "application/json; charset=utf-8" `
  -Body $body
```

SSE 返回格式：每个 `data:` 行是一个 JSON 事件，`done:false` 为中间 token，`done:true` 结束时包含 `sessionId`。Nginx 代理时必须配置 `proxy_buffering off` 才能逐 token 推送。
